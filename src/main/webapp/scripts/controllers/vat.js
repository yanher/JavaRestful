angular.module('VATControllerModule', []).controller('VATController', ['$rootScope', '$scope', '$timeout', '$localStorage', '$state', 'UserService', 'InvoiceService', 'Utility', 'uiGridConstants', function($rootScope, $scope, $timeout, $localStorage, $state, UserService, InvoiceService, Utility, uiGridConstants) {
    ...
    $scope.gridOptions = {
        ...
        data: $rootScope.allVATContracts && $rootScope.allVATContracts.filter(function(val) {
        ...
        onRegisterApi: function(gridApi) {
            ...
            $scope.gridApi.core.on.filterChanged($scope, function() {
                ...
                if (grid.columns[0].filters[0].term) {
                    ...
                    $scope.gridOptions.data = $rootScope.allVATContracts.filter(function(val) {
                        return cs.indexOf(val.CONTRACT_NUMBER) > -1 || val.CONTRACT_NUMBER.indexOf(cs) > -1;
                    });
                } else {
                    $scope.gridOptions.useExternalFiltering = false;
                    $scope.gridOptions.data = $rootScope.allVATContracts && $rootScope.allVATContracts.filter(function(val) {
                        return val.OBJECT_STATUS === 10;
                    }) || [];
                }
            });
    ...
    $scope.detailVAT = function(id) {
        $scope.theContract = $rootScope.allVATContracts.filter(function(val) {
            return val.ID === id;
        })[0];
    };
    ...
    if (!$rootScope.allVATContracts) {
        $rootScope.showLoader.show = true;
        InvoiceService.getAllContracts().success(function(data, status) {
            $rootScope.showLoader.show = false;
            $rootScope.allVATContracts = angular.fromJson(data.d).contracts.filter(function(val) {
                return val.INVOICE_TYPE == 'VAT';
            });
            $scope.gridOptions.data = $rootScope.allVATContracts;
            $rootScope.allBTContracts = angular.fromJson(data.d).contracts.filter(function(val) {
                return val.INVOICE_TYPE == 'BT';
            });
            $rootScope.updateTime = (new Date()).toString();
        }).error(function(data, status) {
